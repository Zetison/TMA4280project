...
strength = [ 1;
             1;
            -2];
A = [3/2-sqrt(2), -1/sqrt(2), -1/2+sqrt(2);
     -2-1/sqrt(2), 1, 2-1/sqrt(2);
     -1/2-sqrt(2), -1/sqrt(2), 3/2+sqrt(2)];
A = [A zeros(3,1) -fliplr(A) zeros(3,1);
     zeros(1,8);
     -flipud(A) zeros(3,1) rot90(A,2) zeros(3,1);
     zeros(1,8)];
    ...
    while true
        ...
        for m = 0:N
            ...
            if false
                ...
                if mod(m+1,4)*mod(n+1,4) == 0
                    n = n - 1;
                    continue
                end
                fact_x = (m+1)*pi;
                fact_y = (n+1)*pi;
                a_mn = 4/(fact_x^2+fact_y^2)*A(mod(m,8)+1, mod(n,8)+1);       
            end
...
export_fig('exact7', '-png', '-transparent', '-r200')
% N = 10;
% A = zeros(N+1,N+1);
% for m = 0:N
%     for n = 0:N
